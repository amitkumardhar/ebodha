...
from app.models.examination import Registration, Examination, Marks
from app.core import security

# Use in-memory SQLite for testing
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_course_details.db"
...
def setup_data():
    ...
    # Register Student
    reg = Registration(student_id="s1", course_offering_id=o1.id, grade="A", grade_point=10.0)
    db.add(reg)
    db.commit()
    db.refresh(reg)
    
    # Create Exam and Marks
    exam = Examination(course_offering_id=o1.id, name="Midterm", max_marks=50.0)
    db.add(exam)
    db.commit()
    db.refresh(exam)
    
    marks = Marks(registration_id=reg.id, examination_id=exam.id, marks_obtained=45.0)
    db.add(marks)
    db.commit()
...
def verify():
    ...
    response = client.get(f"/api/v1/courses/semester-course-details?semester_id={semester_id}&course_code=CS101", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["student_id"] == "s1"
    assert data[0]["marks"][0]["marks_obtained"] == 45.0
    print("Admin sees student details: PASS")
    
    print("Verifying Assigned Teacher Access...")
    ...
    data = response.json()
    assert len(data) == 1
    print("Assigned teacher sees student details: PASS")
    
    print("Verifying Unassigned Teacher Access...")
    token = security.create_access_token("t2", role="teacher")
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(f"/api/v1/courses/semester-course-details?semester_id={semester_id}&course_code=CS101", headers=headers)
    assert response.status_code == 403
    print("Unassigned teacher is forbidden: PASS")
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        with open("traceback_details.log", "w") as f:
            traceback.print_exc(file=f)
        print(f"Verification failed: {e}")
        sys.exit(1)
    finally:
        if os.path.exists("./test_course_details.db"):
            os.remove("./test_course_details.db")
